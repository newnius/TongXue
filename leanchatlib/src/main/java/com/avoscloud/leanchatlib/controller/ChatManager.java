...
public class ChatManager extends AVIMClientEventHandler {
  ...
  public void fetchConversationWithUserId(String userId, final AVIMConversationCreatedCallback callback) {
    ...
    /**added by Super pan*/
    public void fetchConversationWithGroupName(String groupId, final AVIMConversationCreatedCallback callback) {

        final String groupName =groupId;
        final List<String> members = new ArrayList<>();
        members.add(selfId);
        AVIMConversationQuery query = imClient.getQuery();
        query.whereEqualTo(ConversationType.ATTR_GNAME_KEY, groupName);
        query.orderByDescending(KEY_UPDATED_AT);
        query.findInBackground(new AVIMConversationQueryCallback() {
            @Override
            public void done(List<AVIMConversation> conversations, AVException e) {
                if (e != null) {
                    ...
                    if (conversations.size() > 0) {
                        final AVIMConversation conversation= conversations.get(0);

                        if(conversation.getMembers().contains(selfId)){
                        ...
                        Map<String, Object> attrs = new HashMap<>();
                        attrs.put(ConversationType.TYPE_KEY, ConversationType.Single.getValue());
                        attrs.put(ConversationType.GNAME_KEY, groupName);
                        imClient.createConversation(members, attrs, callback);
                    }
                }
            }
